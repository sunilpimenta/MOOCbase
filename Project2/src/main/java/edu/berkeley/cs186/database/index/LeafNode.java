...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        int limit = (int) Math.ceil(2 * metadata.getOrder() * fillFactor);
        while (data.hasNext() && keys.size() <= limit) {
            ...
            if (keys.contains(key)) {
                throw new BPlusTreeException("Duplicate Key was used");
            }
        ...
        Optional<Pair<DataBox, Long>> newPair = Optional.empty();
        if (keys.size() > limit) {
            ...
            this.rightSibling = Optional.of(pageNum);
            newPair = Optional.of(new Pair<>(newKeys.get(0), pageNum));
        }
        sync();
        return newPair;
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Long s = buf.getLong();
        Optional<Long> rightSibling = s == -1 ? Optional.empty() : Optional.of(s);
