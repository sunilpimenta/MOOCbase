...
public class SortOperator {
    ...
    public Run sortRun(Run run) throws DatabaseException {
        ...
        ls.sort(this.comparator);
        SortOperator s = new SortOperator(transaction, tableName, this.comparator);
    ...
    public Run mergeSortedRuns(List<Run> runs) throws DatabaseException {
        ...
        for (int i = 0; i < runs.size(); i ++) {
            ...
            Iterator<Record> it = currRun.iterator();
            while (it.hasNext()) {
        ...
        SortOperator.Run r = s.createRun();
        while (!pq.isEmpty()) {
            r.addRecord(pq.remove().getFirst().getValues());
        }
        return r;
    }
    ...
    public List<Run> mergePass(List<Run> runs) throws DatabaseException {
        ...
        int numChunk = runs.size() / availableBuffers;

        for (int i = 0; i < numChunk; i ++) {
            List<Run> inMemoryRuns = new ArrayList<>();
            for (int j = 0; j < availableBuffers; j++) {
                inMemoryRuns.add(runs.get(numChunk * i + j));
            }
            Run runSorted = mergeSortedRuns(inMemoryRuns);
            ret.add(runSorted);
        }

        if (runs.size() > numChunk * availableBuffers) {
            List<Run> inMemoryRuns = new ArrayList<>();
            for (int i = numChunk * availableBuffers; i < runs.size(); i++) {
                inMemoryRuns.add(runs.get(i));
            }
            Run runSorted = mergeSortedRuns(inMemoryRuns);
            ret.add(runSorted);
        }
    ...
    public String sort() throws DatabaseException {
        ...
        Iterator dataSource = transaction.getPageIterator(tableName);
        while (dataSource.hasNext()) {
            BacktrackingIterator runBlock = transaction.getBlockIterator(tableName, dataSource, numBuffers);
            Run r = createRunFromIterator(runBlock);
            r = sortRun(r);
            runs.add(r);
        }
