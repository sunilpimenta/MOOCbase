    ...
    private class BNLJIterator extends JoinIterator {
        ...
        private void fetchNextLeftBlock() {
            ...
            while (leftIterator.hasNext()) {
                this.leftRecordIterator = getBlockIterator(getLeftTableName(), leftIterator, usableBuffers);
                if (leftRecordIterator.hasNext()) {
                    leftRecordIterator.markNext();
                    this.leftRecord = leftRecordIterator.next();
                    break;
                } else {
                    // No more records or pages from the left table
        ...
        private void joinLeftRecordRightPage() {
            do {
                if (this.rightRecord != null) {
                    // given one left record, look through the right page
                ...
                // moves to the next record on the right, regardless of joining results.
                this.rightRecord = rightRecordIterator.hasNext() ? rightRecordIterator.next() : null;
            } while (!this.hasNext() && rightRecordIterator.hasNext());
        }

        /*resetLeftBlock should be used when we have looked through all records on a right page
         */
         ...
         */
        private void fetchNextRecord() throws NoSuchElementException {
            nextRecord = null;

            if (rightRecord != null) {
                joinLeftRecordRightPage();
            }

            while (!hasNext() && leftRecordIterator.hasNext()) {
                leftRecord = leftRecordIterator.next();
                resetRightPage();
                joinLeftRecordRightPage();
            }
            // we are at the end of left block
            while (!hasNext() && rightIterator.hasNext()) {
                fetchNextRightPage();
                resetLeftBlock();
                joinLeftRecordRightPage();
            }
            // we are at the end of rightBlock
            while (!hasNext() && leftIterator.hasNext()) {
                fetchNextLeftBlock();
                resetRightBlock();
                fetchNextRecord();
            }
            ...
            if (!hasNext()) {
                throw new NoSuchElementException("End of left block. No new record to fetch");
            }
        }
